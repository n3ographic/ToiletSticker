...
const ADMIN_KEY = import.meta.env?.VITE_ADMIN_KEY || window.VITE_ADMIN_KEY;
...
// lights
scene.add(new THREE.HemisphereLight(0xffffff, 0x1b1e22, 0.45));
...
function loadTexture(url){ return new Promise((res, rej)=> texLoader.load(url, (t)=>res(t), undefined, rej)); }

// ---------- Supabase helpers ----------
async function fetchCountLeft(){
  if (!sb) return {left:2, blocked:false};
  const since = new Date(Date.now()-24*3600e3).toISOString();
  const { count } = await sb.from('stickers')
...
async function publishSticker(){
  ...
  if (blocked) { btnPublish.disabled = true; btnPublish.textContent = 'Blocked'; notify('Limit reached: 2 stickers / 24h'); return; }

  const file = fileInput.files?.[0];
  if (!file) { notify('Choose an image first.'); return; }

  const fileName = `users/${sessionId}/${crypto.randomUUID()}-${file.name}`;
  const { error:upError } = await sb.storage.from('stickers').upload(fileName, file, { upsert:false, contentType:file.type });
  ...
  const payload = {
    ...
    image_url: imageUrl,
    position: [hoverPos.x, hoverPos.y, hoverPos.z],
    quaternion: [hoverQuat.x, hoverQuat.y, hoverQuat.z, hoverQuat.w],
    scale: hover.scale.x,
    rotz: rotZ,
...
async function loadPersistedStickers(){
  if (!sb) return;
  const { data } = await sb.from('stickers').select('*').order('created_at');
...
canvas.addEventListener('pointermove', (e)=>{ setMouse(e); updateHover(); });
canvas.addEventListener('click', ()=>{ if (hover) notify('Sticker placé (non publié)'); });
...
sizeInput.addEventListener('input', ()=>{ if (hover) hover.scale.setScalar(Number(sizeInput.value)); });
rotInput.addEventListener('input', ()=> updateHover());
btnRemove.addEventListener('click', ()=>{ if (hover) { scene.remove(hover); hover=null; }});
btnPublish.addEventListener('click', publishSticker);

// Admin
document.addEventListener('keydown', (e)=>{ if (e.key.toLowerCase()==='a') adminDialog.showModal(); });
...
async function boot(){
  try { await loadModel(); } 
  catch(e){ notify('Model load error — check /toilet.glb'); console.error(e); }
  if (sb){
    const { left, blocked } = await fetchCountLeft();
    publishCount = 2-left;
